...
int main()
{
    ...
    int d = 4;
    int b = 4;
    SoundBuffer sbmainsound;
    Sound mainsound;  
    sbmainsound.loadFromFile("audio/mainSound.flac");
    mainsound.setBuffer(sbmainsound);
    ...
    Enemy beerboss(2, "beer.png");
    Texture tBeerBullet;
    Sprite BeerBullet;
    tBeerBullet.loadFromFile("image/beerbull.png");
    BeerBullet.setTexture(tBeerBullet);
    bool shot = false;
    bool side = true;
    bool eb = false;
    double hp = 10;
    ...
    bool go = false;
    bool bbd = false;
    if (!sbmainsound.loadFromFile("audio/mainSound.flac")) {
        window.close();
        window.clear();
        window.display();
    }
    mainsound.play();
    shapka.setPosition(2000, 2000);
    while (window.isOpen()) {
        ...
        if (go == true) {
            if (Keyboard::isKeyPressed(Keyboard::O)) {
                ...
                go = false;
                person = 1;
                boss1.sprite.setPosition(1000, 800);
        ...
        if (bull.getGlobalBounds().intersects(beerboss.sprite.getGlobalBounds()) && beerhp > 0 && go == false) {
            beerhp -= 1;
            bull.setPosition(2000, 2000);
        }
        if (shapka.getGlobalBounds().intersects(beerboss.sprite.getGlobalBounds()) && lyeShapka == true) {
            beerhp -= 1;
            lyeShapka = false;
        }
        ...
        if (beerboss.sprite.getGlobalBounds().intersects(sudo.sprite.getGlobalBounds()) && person == 2 && go == false && hp == 0) {
            go = true;
        }
        if (eb = true && go == false) {
        ...
        if (hp == 0 && beerhp != 0) {
            beerboss.sprite.move(beerx, beery);
        }
        if (beerboss.sprite.getPosition().y >= 800 && beerboss.sprite.getPosition().x <= 0) {
            beerboss.sprite.setPosition(0, 800);
            if (randomInt(1, 3) == 1) {
                beerboss.sprite.move(0, -4);
                beerx = 0;
                beery = -4;
            }
            ...
            else if (randomInt(1, 3) == 3) {
                beerboss.sprite.move(4, 0);
                beerx = 4;
                beery = 0;
            }
            
        }
        if (beerboss.sprite.getPosition().y >= 800 && beerboss.sprite.getPosition().x >= 1700 ) {
            beerboss.sprite.setPosition(1700, 800);
            if (randomInt(1, 3) == 1) {
                beerboss.sprite.move(0, -4);
                beerx = 0;
                beery = -4;
        ...
        if (beerboss.sprite.getPosition().y <= 0 && beerboss.sprite.getPosition().x >= 1700) {
            beerboss.sprite.setPosition(1700, 0);
            if (randomInt(1, 3) == 1) {
                beerboss.sprite.move(-4, 0);
                beerx = -4;
                beery = 0;
        ...
        if (beerboss.sprite.getPosition().y <= 0 && beerboss.sprite.getPosition().x <= 0) {
            beerboss.sprite.setPosition(0, 0);
            if (randomInt(1, 3) == 1) {
                beerboss.sprite.move(0, 4);
                beerx = 0;
                beery = 4;
        ...
        if (hp == 0 && beerhp != 0 && go == false) {
            window.draw(beerboss.sprite);
        }
        if (bbd == true && go == false) {
            window.draw(BeerBullet);
        }
        window.display();
    }
}
